...
class NGSIv2Client:
    ...
    def create_entity(self, id, type, temperature, pressure):
        """
        Create entity
        """
        entity = self.get_entity_model(id=id, type=type, temperature=temperature, pressure=pressure)

        self.client.Entity.create_entity(entity=entity, _request_options=copy.deepcopy(self.options)).result()
    ...
    def set_entity_attrs(self, entity_id, temperature, pressure):
        ...
        self.client.Entity.set_entity_attrs(entity_id=entity_id, attribute_list=new_attr_list,
                                            _request_options=copy.deepcopy(self.options)).result()

        Entity = self.client.get_model('Entity')
        return Entity(id=entity_id, temperature=temperature, pressure=pressure)
